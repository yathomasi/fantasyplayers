...
GWDIR = DIRNAME + "GW" + GWNumber + "/"
# Creating GameWeek dir if not available
if not os.path.exists(GWDIR):
    print("GW dir not resent")
    # exit()
    print("Making"+GWDIR)
    os.makedirs(GWDIR)
# else:
...
while (True):
    try:
        ...
        writeToFile(listOfcountOfplayersPicked, os.path.join(
            GWDIR, "result playersPicked " + str(leagueIdSelected) + ".csv"))
        listOfCountOfCaptainsPicked = sorted(
            countOfCaptainsPicked.items(), key=lambda x: x[1], reverse=True)
        writeToFile(listOfCountOfCaptainsPicked, os.path.join(
            GWDIR, "result captain " + str(leagueIdSelected) + ".csv"))
